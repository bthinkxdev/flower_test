...
class DeliverySlotType(models.TextChoices):
    """Time-of-day classification for a delivery slot."""
...
class DeliverySlot(ArabicTranslationMixin, TimeStampedModel):
    ...
    slot_type = models.CharField(
        max_length=20,
        choices=DeliverySlotType.choices,
        default=DeliverySlotType.MORNING,
        db_index=True,
        verbose_name="Slot type",
    )
    max_capacity_per_day = models.PositiveIntegerField(
        default=100,
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["is_active", "slot_type"], name="delivery_slot_active_idx"),
        ]
...
class DeliverySlotBooking(TimeStampedModel):
    """Tracks how many orders are booked against a slot on a given date."""
    ...
    date = models.DateField(db_index=True, verbose_name="Delivery date")
    current_bookings = models.PositiveIntegerField(
        default=0,
        verbose_name="Current bookings",
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["slot", "date"], name="delivery_slot_date_unique"),
        ]
        indexes = [
            models.Index(fields=["slot", "date"], name="delivery_booking_slot_date_idx"),
        ]

    def __str__(self) -> str:
        return f"{self.slot.name} on {self.date} ({self.current_bookings})"
